...
OvtransformNodeletClass::OvtransformNodeletClass(const rclcpp::NodeOptions& transform_node) : Node("odom_transform_nodelet", transform_node)
{
...
void OvtransformNodeletClass::setup(const std::string transform_config_path) {
    ...
    RCLCPP_INFO(this->get_logger(), "[odom_transform] Publishing: %s",
                pub_odomworldB0->get_topic_name());
    RCLCPP_INFO(this->get_logger(), "[odom_transform] Publishing: %s",
                pub_odomworld->get_topic_name());
...
void OvtransformNodeletClass::setupTransformationMatrix(const std::string transform_config_path) {
    ...
    RCLCPP_INFO(this->get_logger(), "T_cam_imu:");
        for (int i = 0; i < 3; ++i) {
            for (int j = 0; j < 4; ++j) {
                RCLCPP_INFO(this->get_logger(), "  r%d_c%d: %f", i, j, T_cam_imu["r" + std::to_string(i)]["c" + std::to_string(j)].as<double>());
            }
        }
        ...
        for (int i = 0; i < 3; ++i) {
            for (int j = 0; j < 4; ++j) {
                RCLCPP_INFO(this->get_logger(), "  r%d_c%d: %f", i, j, T_cam_body["r" + std::to_string(i)]["c" + std::to_string(j)].as<double>());
            }
        }
        ...
        for (int i = 0; i < 3; ++i) {
            for (int j = 0; j < 4; ++j) {
                RCLCPP_INFO(this->get_logger(), "  r%d_c%d: %f", i, j, T_B0_W["r" + std::to_string(i)]["c" + std::to_string(j)].as<double>());
            }
        }
    ...
    if (init_world_with_vicon) {
        ...
        if (sharedInitBodyOdominW != nullptr) {
            ...
            RCLCPP_INFO(this->get_logger(), "Failed to get init T_BtoW from vicon topic %s, use default T_BtoW by setting init_world_with_vicon false", viconOdomWTopic.c_str());
        }
    }
